...
def get_args():
    ...
    parser.add_argument('-q', '--quiet', help='be vewy vewy quiet',
            action='store_true')
...
def get_excluded(x, sum_file):
    ...
    for arg in re.split('\s*,\s*', x.lower()):
        if str.isdigit(arg):
            if arg in name_to_id.values():
                exclude.add(arg)
            else:
                print('{} is not a valid taxid'.format(arg))
        elif arg in name_to_id:
            exclude.add(name_to_id[arg])
        else:
            ids = [name_to_id[x] 
                    for x in name_to_id.keys() if re.match(arg, x)]
...
def main():
    ...
    exclude_fh = open(os.path.join(exclude_dir, basename), 'wt') \
